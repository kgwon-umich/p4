...
class Post {
  public:
    ...
    const set<string>* get_unique_words() const {
        return &words;
    }
    bool has_word(const string& word) const {
        return words.find(word) != words.end();
    }
    const string& get_content_string() const {
        return content;
    }
...
class dataSet {
  public:
    dataSet(csvstream &csvin) {
        ...
        // chrono::duration<double> timeSpent = end - start;
        // cout << "time spent: " << timeSpent.count() << " seconds" << endl;
    }
